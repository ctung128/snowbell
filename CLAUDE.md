...
npx serve .
```
Any static file server works (`python3 -m http.server`, VS Code Live Server, etc.). Serve it over HTTP — don't open `index.html` directly via `file://`; browsers block `<script type="module">` (used by `js/components/*.js`) under that protocol, which silently breaks the gallery and contact form.
...
- `<photo-gallery>` only owns the row-column layout (`:host`, in `css/photo-gallery.css`), fade-in-on-scroll, and dispatching a `photo-select` event on click — it does **not** own `.gallery-block`/`.gallery-row-group`/`.gallery-item` styling. Those classes stay on slotted (light-DOM) elements and are styled by the global `css/styles.css`, because `::slotted()` can only style a slotted element itself, never its descendants — this component physically cannot reach `.gallery-item img` nested inside a slotted `.gallery-block`. Don't try to move that CSS into the component.
...
- `<contact-form>` (`js/components/contact-form.js`) owns the entire form: markup, validation, submit-to-API, and status messaging, all inside its shadow DOM (unlike the gallery, these fields aren't slotted, so their CSS lives in `css/contact-form.css`, not `css/styles.css`). Client-side checks are only for UX; `server/server.js` is the actual validation boundary. Session-type options are declared as `<option slot="subjects">` children of `<contact-form>` in `index.html` and must stay in sync with `ALLOWED_SUBJECTS` in `server/server.js` — the component reads them via `querySelectorAll('option[slot="subjects"]')` and clones them into its own shadow-owned `<select>` rather than relying on native cross-shadow `<select>` slotting, which isn't reliably supported across browsers (Safari/Firefox don't render it).
- **Both components load their shadow-root styling via `<link rel="stylesheet" href="css/photo-gallery.css">` / `css/contact-form.css">`, never an inline `<style>` tag.** `vercel.json`'s CSP has no `'unsafe-inline'` in `style-src`, and CSP applies to a shadow root's own `<style>` elements exactly as it does to the main document — an inline `<style>` injected via `innerHTML` gets silently blocked (no visible error beyond a console warning), while the component's JS-built markup still renders fine, since CSP doesn't touch DOM structure, only style application. That mismatch (structure present, zero styling applied) is exactly what happened the first time these were built as inline `<style>` tags. An external, same-origin stylesheet isn't restricted by `style-src 'self'`, so `<link>` avoids the problem without weakening the CSP. `index.html` preloads both files (`<link rel="preload" as="style">`) so the shadow roots' `<link>`s resolve instantly instead of triggering a fresh fetch.
...
If a component uses Shadow DOM, give its own styling an external stylesheet (`css/<component>.css`) loaded via `<link rel="stylesheet">` inside the shadow root, not an inline `<style>` tag — this site's CSP (`vercel.json`) has no `'unsafe-inline'` for `style-src`, and CSP blocks inline shadow-root styles just as it would in the light DOM, with no visible error and no effect on whether the component's markup itself renders. Add a matching `<link rel="preload" as="style">` for it in `index.html`'s `<head>` to avoid a flash of unstyled content on first paint.
